...
/** QR code display dimensions */
const QR_SIZE = 200

/**
 * Native implementation of the TOTP enrollment screen.
      ...
      <AuthCard title={t('auth.totp.enrollTitle')}>
        ...
        {factor?.totp?.qr_code ? (
          <View style={{ alignSelf: 'center' }}>
            <Image
              source={{ uri: factor.totp.qr_code }}
              style={{ width: QR_SIZE, height: QR_SIZE }}
              accessibilityLabel={t('auth.totp.qrAccessibilityLabel')}
            />
          </View>
        ) : null}
        {factor?.totp?.secret ? (
