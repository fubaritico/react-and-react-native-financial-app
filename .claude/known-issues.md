...
- Budgets are **month-scoped**: the web budgets page + Home query `getBudgetsOptions({ month: getCurrentBudgetMonth() })`, and `get_budgets_with_spent`/`get_balance` filter transactions by `to_char(date,'YYYY-MM')`. On month rollover (or if data sits in another month) the budgets page is **empty** until budgets exist for the current month — there is no "months" feature/switcher yet. Dev fix: `pnpm seed` (shifts all dates to the current month) or `UPDATE budgets.month` + shift transaction dates. See BM `month-scoped-budgets-empty-page`. Tracked as `### Next` item "Months feature".
- `supabase/setup.sql` + `seed.sql` were realigned to the LIVE schema (custom-categories: `categories` + `user_preferences`, no `balances` table, `category_id` FKs, source/external_id/`check(amount<>0)`, 5 RPCs verbatim, RLS). `seed.sql` is the **manual equivalent** of `pnpm seed` (`apps/api/src/supabase/seed.ts` + `data.json`) and shifts dates to the current month. Prefer `pnpm seed` (idempotent reset) over the SQL file. `categories` has RLS enabled with **no policy** in the live DB (service-role API only) — mirrored as-is.
- NavItem/settings review (deferred): `settings.tsx` `handleSubmit`/`handleDeleteAccount` depend on the full `useMutation` object (REACT-001/002) — recreate on mutation state change; fix = depend on the stable `.mutate`/`.mutateAsync`. `NavItem.styles.ts` `lg:h-[54px]` arbitrary value → needs a token. A few pre-existing handlers lack `@param`/`@returns` JSDoc.
- SegmentedControl A11Y-007 (accepted): segment focus ring uses `focus-within:` (not `focus-visible:`) — matches the established pattern in TextInput/DatePicker for input-wrapper components where the interactive surface is the wrapper, not the hidden input.
- SegmentedControl A11Y-003 (accepted): segments use `py-3` (no `min-h`) — real rendered height ≈45px (24px padding + ~21px `leading-normal` body line) ≥ 44px WCAG target. No arbitrary `min-h-[44px]` added (forbidden) and no `min-h-11` token exists.
- SegmentedControl ARCH-002 (accepted): native segment uses an explicit `{ flex: 1 }` style rather than `shared.segmentFill` (`flex-1`) — mirrors the NavItem precedent for the twrnc Pressable flex-propagation quirk.

- SSR navigation latency RESOLVED — web SSR middleware verifies the JWT locally via `getClaims()` (ES256/JWKS) instead of network `getUser()`. Tradeoff (accepted): a revoked-but-unexpired token survives until the next document load (~token TTL); the API `requireAuth` still validates per request, so data access stays gated.
- REACT-004 (deferred): the 5 data routes wrap `<HydrationBoundary>` inside the component that already called `useQuery`. Works in prod; the review's suggested fix is ineffective and a proper hoist is a risky cross-route restructure.
- Netlify SSR logs flooded with `404 No route matches "/assets/*.js.map"` — sourcemaps are deleted post-build (Sentry `filesToDeleteAfterUpload`) and requested by the browser only when DevTools is open → they fall through to the SSR function. Cosmetic. Fix: a CDN rule to 404 `/assets/*.map` before the function.
- Fly API `ECONNRESET` / "socket disconnected before TLS" to `epouch-api.fly.dev` → check Fly **billing / `fly status` FIRST** (the trial ended once and suspended the app). Not a code or auth bug. See BM `fix-fly-api-econnreset-trial-ended`.
