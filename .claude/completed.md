...
- feat(auth): full Supabase auth flow + API error handling + Android emulator fix (a740df4) — Zod validation schemas (loginSchema/signupSchema) + parseValidationErrors() in @financial-app/shared, web login/signup routes with AuthLayout+AuthCard+Alert+clientLoader auth gate in layout+Sidebar sign-out via Navigation item, Expo login/signup in (auth) group with AuthGate (Redirect+useSegments)+sign-out tab listener, bare RN login/signup with RootNavigator conditional auth gate+sign-out tab listener, i18n all auth labels/placeholders/validation EN/FR, logout icon SVG, API error origin prefixes ([AUTH]/[VALIDATION]/[DATABASE]/[NOT_FOUND]/[BUSINESS]), Alert description prop, getErrorMessage utility, API 0.0.0.0 binding + Android usesCleartextTraffic + 10.0.2.2 remap, web auth token race condition fix, Express 5 req.query read-only fix, TextInput autoCapitalize+keyboardType props, Login/Signup tests for bare RN, review score 84/100
- docs(specs): enrich specifications.md (cff08cf, 0a4b1ae) — product vision, data modes (manual vs bank), onboarding flow (6 new screens not in Figma), empty states per screen, GoCardless Bank Account Data integration (provider, flow, API endpoints, DB schema), recurring income+expenses, month navigation, Settings screen (balance edit, bank accounts, account deletion, language), 15 business rules, 8 open questions, CRUD summary, non-functional requirements
- fix(ui): dropdown scroll hint at 6.5 items (cff08cf) — replaced max-h-60 (240px) with calculated maxHeight (294px = 6.5 × 44px + 8px padding) in ListboxList web + native so 7th item peeks at half-height per Figma spec
- chore: remove unused concurrently dependency (f4daa69) — Turbo handles parallel execution
- chore: regenerate pnpm-lock.yaml (faca4d7)
