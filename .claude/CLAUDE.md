...
2. **Onboarding** — `docs/plans/onboarding-plan.md` ← IN PROGRESS
   ...
   - ~~Custom categories session 5a (CategoriesScreenView polish)~~ ✅ — Storybook stories web (5: Default, SystemOnly, Empty, Deleting, Adding) + native (4: IPhone/Android variants). Back button: raw `<button>` → IconButton atom (new component: composes Button internally, own CVA variants circular/sizes/colors, `packages/ui/src/components/atoms/IconButton/`, registered in both barrels — tests+story pending). Icon color system refactor: shared `semanticColors.ts` (`packages/ui/src/lib/semanticColors.ts`) single source of truth for Icon+Typography color props — `color` prop changé de `string` à `SemanticColor | 'currentColor'`, Icon.web.tsx `resolveCssColor()` → `var(--color-${token})`, Icon.native.tsx `resolveNativeColor()` → `resolveColor(token)` → hex, Typography CVA color variants dérivés du map partagé, ~35 fichiers migrés (raw hex `#FFFFFF`/`tw.color()`/`resolveColor()`/`var(--color-*)` → tokens sémantiques `"on-dark"`/`"muted"`/`"success"` etc.), ajouté `nav-accent`+`grey-300` au map, Button.constants `ICON_COLOR_TOKEN` typé `IconColor`, Alert.constants `SEVERITY_TOKEN` typé `SemanticColor`, PasswordRulesList `ICON_COLOR_MAP` migré, imports `resolveColor` nettoyés dans BudgetCategoryCard/PotCard/CategoryDropdown, Storybook Icon stories migrés hex→tokens. CategoriesScreenView.styles.ts: removed `backButton` class. SOLID React skill créé (`/solid-react-principles`). type-check ✅ lint ✅ test ✅ (163 pass). PENDING: IconButton tests+story, `/review`, `/commit` pour tout le batch.
   - **Next coding**: Custom categories session 5b — `/review` + `/commit` du batch actuel, IconButton tests+story, puis passe design web CategoriesScreenView (user review écran), puis adaptation native, navigation wiring
